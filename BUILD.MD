...
Lindsay's v2 handoff is now the canonical short form for the workflow: ten agents plus `row_scorer_v2`, scoring trust rather than field presence. The scoring question is: is this row coherent, evidenced, geospatially plausible, non-duplicative, and safe to count in planning?

For a three-minute demo, the arc is:

1. Start with the Geographic Score Heatmap and show that uncertainty has a location, not just a dashboard number.
2. Use Mission Control and Row Uncertainty Distribution to explain which rows are trusted enough to count.
3. Drop in or stage new data from a file, website scrape, or update feed.
4. Run the agent pipeline: ingest, QA, dedupe, evidence, geo, shortage, review gate, risk.
5. Show the system found the problems automatically and separated safe fixes from human-review items.
6. Show the downstream planning effect: which places, doctors, facilities, or specialties now look risky because the cleaned/trust-weighted data says so.

```mermaid
flowchart LR
  story[3 minute demo story] --> track4[Track 4: Data Readiness Desk]
  track4 --> heatmap[Geographic trust map]
  heatmap --> agents[Agent-led data cleanup]
...
1. **Open Dashboard**
   - Show Geographic Score Heatmap first.
   - Explain that A/B/C/D row tiers determine whether records can count in planning.
   - Show Mission Control and Row Uncertainty Distribution.
...
Presenter-ready demo materials now live in `demo/`:

- `demo/DEMO_NARRATIVE.md`: high-level story and judging frame.
- `demo/DEMO_SCRIPT.md`: timed click path and speaker notes.
- `demo/DEMO_CHECKLIST.md`: live-readiness checklist before presenting.
- `docs/agent_workflow_pipeline_v2_lindsay_handoff.md`: v2 handoff language for the ten-agent trust pipeline, row scorer, and heatmap.
...
2. "The app opens on a Geographic Score Heatmap, because facility trust has a geography before it becomes a planning recommendation."
3. "Mission Control gives the roll-up, and Row Uncertainty Distribution shows how many rows are A/B countable versus C/D review work."
4. "Now we drop in more data. For the demo, we use `demo/data_readiness_demo_import.xlsx`, a 12-row XLSX with exact duplicates, near duplicates, sparse locations, weak NICU/emergency claims, and suspicious metadata."
5. "Instead of asking a human to clean rows manually, the Databricks agent workflow does the first pass: ingestion, QA, dedupe, evidence extraction, geo checks, shortage analysis, and a review gate."
6. "The output is a proof/reject queue: safe fixes can be accepted quickly, risky changes are held for human confirmation, and every item carries evidence and confidence."
7. "As a side effect of that trusted resulting state, the risk planner shows where care gaps may be real versus where we are just looking at bad or sparse data."
8. "That is the end-to-end promise: Track 4 cleanup creates the Track 2 planning surface."
...
- Narrative: `demo/DEMO_NARRATIVE.md`
...
- All ten agents completed, including PIN directory and NFHS survey context checks.
- Ingestion reported 3 row-quality flags.
- Dedup reported 4 exact duplicate decisions.
- HumanReviewGate reported 50 review items after adding PIN/NFHS guardrails and crossed the material planning impact threshold.

## UX and UI Mockups

The app should feel like a Databricks-native operational dashboard: dense, calm, evidence-first, and optimized for repeated review work. The MVP uses **four top-level tabs** instead of a deep navigation tree:
...
This keeps the hackathon story clean: first show the Geographic Score Heatmap and Current State trust summary, then import/stage data and run the cleansing pipeline, then show what the agents found and what needs proof/reject, then show what the cleaned evidence means for planning.
...
- Default to Unity Catalog source and resulting state in Databricks Apps.
- Keep a local/offline mode for demos and development, but do not silently present the 3-row demo fallback as live DBX data.
- Disable Databricks SQL cloud fetch in the app runtime because the Databricks App container can fail to retrieve result chunks from cloud storage URLs.
- Prewarm `/api/state` on app startup so the 10,000-row source state is loaded before the user clicks around.
...
- Use status colors sparingly: lava/red for critical or primary action, amber for review, green for trusted/completed, gray for duplicate/unknown, and blue-gray for neutral information.
...
Purpose: This is the landing page. It shows the current dataset, Geographic Score Heatmap, current numbers, data readiness score, row uncertainty distribution, executive summary, and dataset preview before planning.
...
| up to 10,000 valid mapped rows; colored by A/B/C/D row uncertainty tier           |
+----------------------------------------------------------------------------------+
| Records 10,000    | States 28         | Districts 612     | Sparse Locations 931 |
+-------------------+-------------------+-------------------+----------------------+
| Mission Control                              | Row Uncertainty Distribution       |
...
- Render Geographic Score Heatmap above Mission Control.
- Render Row Uncertainty Distribution beside Mission Control instead of generic queue copy.
- Use CSS progress bars for score components.
- Use a table component for the current dataset preview.
- Dataset preview is a sample view, not the full dataset table. The UI labels it as "showing N preview rows from 10,000 loaded facility records."
- Readiness flags render as chips:
  - red for missing or sparse fields
  - gray for duplicate/cluster signals
  - blue-gray for neutral/ok signals
...
- User runs the ten-agent pipeline.
- Agent cards show ingestion, QA, PIN directory, NFHS survey, dedupe, evidence, geo, shortage, review gate, and risk status.
...
- Use React tabs, queue lanes, and filters for queue segments.
...
- Queue items carry workflow metadata: queue, next step, primary action, secondary action, assignee, decision-required flag, evidence, confidence, and lift.
...
- Background: Oat Light `#F9F7F4` and white panels.
- Text: Navy 900 `#0B2026`; secondary text Gray Text `#5A6F77`.
- Primary action/selected state: Lava 600 `#FF3621`; hover Lava 500 `#FF5F46`.
- Navigation and quiet structure: Navy 800 `#1B3139`, Gray Navigation `#303F47`, Gray Lines `#DCE0E2`.
- Status colors: Green 600 `#00A972`, Yellow 600 `#FFAB00`, Maroon 600 `#98102A`, Blue 600 `#2272B4`.
...
- Chips for readiness flags, queue state, backend state, scratchpad tags, and status labels.
...
- DBX source/state load failure: fail loudly with diagnostics. Do not show the tiny demo dataset as if it is the live 10,000-row source.
- Databricks SQL result fetch failure from cloud storage URLs: set `DATABRICKS_SQL_USE_CLOUD_FETCH=false`.
...
- Geographic Score Heatmap with up to 10,000 valid India lat/lon rows, C/D row count, clickable A/B/C/D legend, zoom controls, and row-tier colored points.
...
- Dataset preview with readiness flags plus search, sort, tier filter, and state filter.
...
Current UI shape: selecting a risk opens an action-style detail panel with a recommendation statement, next step, evidence/facts to verify, linked cleanup action cards, and planner notes. Risk rows and map dot pills must route back to the underlying cleanup queue so planners can see whether a gap is real or data-poor.
...
- Linked cleanup actions for location, evidence, dedupe, and row uncertainty blockers.
...
### White-Label Dataset Pack Architecture

The app should become a reusable Data Readiness Desk shell. Dataset-specific logic belongs in dataset packs, not scattered through the React app or FastAPI routes.

```mermaid
flowchart TB
  subgraph CORE[Core Product]
    tabs[Current State / Import + Pipeline / Actions / Risk]
    api[FastAPI state + action APIs]
    cache[Hot cache and diagnostics]
    runner[Agent pipeline runner]
    ucmodel[Source / work / result / audit model]
  end

  subgraph PACK[Dataset Pack]
    sourceCfg[Source table config]
    mapping[Canonical schema mapping]
    rules[Quality and evidence rules]
    specs[Agent specs and prompts]
    scores[Score guide and labels]
  end

  india[India DAIS facilities pack] --> PACK
  zimbabwe[Future Zimbabwe facilities pack] --> PACK
  future[Future health dataset pack] --> PACK

  PACK --> runner
  PACK --> ucmodel
  PACK --> tabs
  CORE --> dbx[Databricks App]
```

Dataset pack contract:

- `dataset_id`, display name, country/region, and planning domain.
- Unity Catalog source tables and result catalog/schema targets.
- Canonical field map into the app's normalized facility/result state.
- Quality rulebook docs and score definitions.
- Agent list, workflow refs, and rule-family labels.
- Optional country-specific reference tables such as PIN/postcode directories, district surveys, or administrative boundaries.
- Demo narrative/copy overrides so the app can be white-labeled without changing core code.
...
- **PincodeIngestionAgent**: validates the PIN directory workflow contract, enforces join-safe lookup posture, and surfaces PIN enrichment review items.
- **NfhsSurveyIngestionAgent**: validates the NFHS-5 district survey workflow contract, enforces normalized district join keys, and preserves suppressed/caution caveats.
...
The merged fork docs are now integrated as the agent workflow rulebook:

- `agents/ingestion_agent.md`: ingestion orchestrator, alignment/cleaning, dedupe, review surface, and scoring agent prompts.
- `docs/facilities_data_quality.md`: concrete field-level quality rules, dedupe classes, canonical state mapping, geocoding strategy, and scoring baseline.
- `agents/pincode_ingestion_agent.md`: PIN directory enrichment orchestrator for post-office cleaning, coordinate parsing, PIN aggregation, ambiguity flags, and scoring.
- `docs/pincode_data_quality.md`: concrete PIN directory rules, including the key guardrail that a PIN code is not a district key.
- `agents/nfhs_survey_ingestion_agent.md`: NFHS-5 district survey ingestion orchestrator for schema checks, geography normalization, indicator parsing, caveat flags, and ingestion scoring.
- `docs/nfhs_survey_ingestion_data_quality.md`: concrete NFHS rules, including the guardrails that suppressed `*` values are null, caution estimates stay flagged, and ingestion quality is not district health risk.
- `app/lib/agents/SPEC.md`: current implementation contract that maps those rules into the ten app agents.

Runtime agent payloads now include `workflow_ref` and `rule_families`, and the React pipeline cards display those rule families so the demo shows real workflow substance, not generic agent boxes.

PIN enrichment posture:

- The raw India Post PIN directory is post-office grain.
- Facilities must join only to `pincode_lookup_clean`, the one-row-per-PIN aggregate.
- Multi-state and weak multi-district PINs must create ambiguity flags and review items.
- PIN-derived coordinates are approximate postal centroids, not exact facility geocodes.

NFHS survey posture:

- NFHS is district-level survey context, not facility evidence.
- Suppressed `*` values become null plus caveat flags; they are never zeros.
- Parenthesized estimates remain available with visible caution flags.
- NFHS context joins require normalized state/district keys and cannot validate individual facility claims.

Current deploy status:

- Local agent mode is implemented.
- Local ten-agent skeleton was revalidated on 2026-06-15: Python compile passed, frontend build passed, and a local pipeline run completed `ingestion`, `qa`, `pincode`, `nfhs`, `dedup`, `evidence`, `geo`, `shortage`, `review`, and `risk`.
...
- Databricks multi-task Job exists as `590750946177761`, but Job mode is **not verified deployed** until an end-to-end run completes.
...
- Vite/React frontend for the rich four-tab workflow.
...
- Hot in-memory `/api/state` cache with startup prewarm and background refresh.
- Strict DBX behavior: Unity Catalog load failures should be visible diagnostics, not a silent 3-row demo fallback.
...
- Mock profiling, actionable queue recommendation, and risk generator in `app/lib/reparser.py`.
...
8. React UI refreshes the four tabs from `GET /api/state`.
9. The Actions tab presents queue lanes for human review, agent-ready fixes, evidence review, steward triage, and closed decisions.
10. Selected action decisions are written through `POST /api/actions/{action_id}/decision`; in Unity Catalog state mode they update `result.action_recommendations` and append `result.action_decisions` plus audit events.
11. Dataset preview shows a bounded sample with color-coded readiness chips and a label indicating total loaded records.
...
- `/api/state` prewarms on FastAPI startup.
- Once live state is cached, `/api/state` is cache-first and refreshes in the background.
- In DBX mode, source/state load failures fail loudly with diagnostics. The app must not show the 3-row demo fallback as live data.
- The tiny demo fallback is reserved for local/offline click-through or explicitly enabled fallback mode.
- Databricks SQL connector runs with `use_cloud_fetch=false`; Databricks Apps may not be able to fetch result chunks from external cloud storage URLs.
...
Latest DBX data-load validation on 2026-06-15:

- Source: `databricks_virtue_foundation_dataset_dais_2026.virtue_foundation_dataset.facilities`.
- Warehouse: `e428febecc2419c5`.
- With `DATABRICKS_SQL_USE_CLOUD_FETCH=false`, the app state builder loaded 10,000 rows and 51 columns.
- Full app state validation produced `backend=live`, `fallback=False`, and 100 preview rows.
- The cloud-fetch failure mode was observed in Databricks App logs as blocked requests to `us-west-2.storage.cloud.databricks.com`.
